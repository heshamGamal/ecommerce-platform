...
final class DeleteAttribute {public function __construct(private readonly AttributeRepositoryInterface $attributes){} public function execute(int $id):void{$a=$this->attributes->findOrFail($id);if($this->attributes->isUsed($a))throw new BusinessRuleException('An attribute used by variants cannot be deleted.');$this->attributes->delete($a);}}
