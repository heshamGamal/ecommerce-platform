...
final class DeleteAttributeValue {public function __construct(private readonly AttributeValueRepositoryInterface $values){} public function execute(int $attributeId,int $id):void{$v=$this->values->findOrFail($id,$attributeId);if($this->values->isUsed($v))throw new BusinessRuleException('An attribute value used by variants cannot be deleted.');$this->values->delete($v);}}
