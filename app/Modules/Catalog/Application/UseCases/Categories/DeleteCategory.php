...
final class DeleteCategory {public function __construct(private readonly CategoryRepositoryInterface $categories){} public function execute(int $id):void{$c=$this->categories->findOrFail($id);if($this->categories->hasProductsOrChildren($c))throw new BusinessRuleException('A category containing products or child categories cannot be deleted.');$this->categories->delete($c);}}
