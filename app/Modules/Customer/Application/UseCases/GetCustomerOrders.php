...
final class GetCustomerOrders { public function __construct(private readonly AuthenticationServiceInterface $auth,private readonly CustomerFeaturesRepositoryInterface $repo){} public function execute():iterable{$u=$this->auth->user();if(!$u)throw new AuthenticationException('Unauthenticated.');return $this->repo->orders($u->id);} }
