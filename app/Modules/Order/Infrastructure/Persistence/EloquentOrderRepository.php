...
final class EloquentOrderRepository implements OrderRepositoryInterface
{
    ...
    public function cancelForUser(int $userId, int $orderId): CustomerOrder
    {
        return DB::transaction(function () use ($userId, $orderId): CustomerOrder {
            ...
            foreach ($order->items as $item) {
                $inventory = InventoryItem::query()->where('product_id', $item->product_id)->whereNull('variant_id')->lockForUpdate()->first();
    ...
    public function checkout(int $userId, int $addressId, string $currency, ?string $idempotencyKey): CustomerOrder
    {
        return DB::transaction(function () use ($userId, $addressId, $currency, $idempotencyKey): CustomerOrder {
            ...
            $user = User::query()->with(['cart.items.product', 'addresses'])->findOrFail($userId);
            $cart = $user->cart;
            ...
            foreach ($items as $cartItem) {
                ...
                if ($product->price === null || $product->price < 0) throw CheckoutException::missingPrice($product->name);
                $inventory = InventoryItem::query()->where('product_id', $product->id)->whereNull('variant_id')->lockForUpdate()->first();
                ...
                $subtotal += $lineTotal;
                $snapshots[] = ['product_id' => $product->id, 'name' => $product->name, 'sku' => null, 'quantity' => $cartItem->quantity, 'unit_price' => $product->price, 'discount_amount' => 0, 'tax_amount' => 0, 'total_amount' => $lineTotal];
