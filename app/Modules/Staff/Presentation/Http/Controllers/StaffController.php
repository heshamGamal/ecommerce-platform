...
final class StaffController extends Controller
{
 ...
 public function update(StaffRequest $request,int $id,StaffRepositoryInterface $repository,UpdateStaff $useCase):JsonResponse{return response()->json(['data'=>$useCase->execute($repository->find($id),StaffData::fromArray($request->validated()))]);}
 public function destroy(StaffRequest $request,int $id,StaffRepositoryInterface $repository,DeleteStaff $useCase):JsonResponse{$useCase->execute($repository->find($id));return response()->json(['message'=>'Staff deleted successfully.']);}
}
