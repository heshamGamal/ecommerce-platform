...
final class ProductReviewApiTest extends TestCase
{
    ...
    public function test_verified_customer_can_submit_and_admin_can_moderate_review(): void
    {
        ...
        $review = $this->actingAs($customer)->postJson('/api/products/'.$product->id.'/reviews', ['rating' => 5, 'title' => 'Excellent', 'body' => 'Great product.'])->assertCreated()->assertJsonPath('data.status', 'pending')->json('data.id');
        $this->actingAs($admin)->patchJson('/api/reviews/'.$review.'/moderate', ['status' => 'approved'])->assertOk();
        $this->actingAs($customer)->getJson('/api/products/'.$product->id.'/reviews')->assertOk()->assertJsonPath('meta.count', 1)->assertJsonPath('meta.average_rating', 5);
        $this->assertDatabaseHas('product_reviews', ['id' => $review, 'verified_purchase' => true, 'status' => 'approved']);
        $this->actingAs($customer)->postJson('/api/products/'.$product->id.'/reviews', ['rating' => 4])->assertStatus(409);
    }
    public function test_customer_cannot_review_without_a_delivered_purchase(): void
    {
        ...
        $this->actingAs($customer)->postJson('/api/products/'.$product->id.'/reviews', ['rating' => 5])->assertStatus(409);
    }
