...
class ProductApiTest extends TestCase
{
    ...
    public function test_product_crud_lists_relations_and_returns_404(): void
    {
        ...
        $create = $this->actingAs($this->admin)->postJson('/api/products', [
        ...
        $this->getJson('/api/products')->assertOk()->assertJsonPath('data.0.id', $id);
        $this->getJson("/api/products/{$id}")->assertOk()->assertJsonPath('data.category.id', $category->id);
    ...
    public function test_variant_crud_and_nested_404_boundaries(): void
    {
        ...
        $create = $this->actingAs($this->admin)->postJson("/api/products/{$product->id}/variants", $this->variantPayload('SKU-1', [$colorRed->id]))
            ->assertCreated()->assertJsonPath('data.sku', 'SKU-1');
        $variantId = $create->json('data.id');

        $this->getJson("/api/products/{$product->id}/variants")->assertOk()->assertJsonCount(1, 'data');
        $this->getJson("/api/products/{$product->id}/variants/{$variantId}")->assertOk()->assertJsonPath('data.id', $variantId);
        $this->patchJson("/api/products/{$product->id}/variants/{$variantId}", $this->variantPayload('SKU-1-UPDATED', [$colorRed->id], 250))
            ->assertOk()->assertJsonPath('data.price', 250);

        $other = Product::query()->create(['name' => 'Other', 'slug' => 'other', 'type' => 'variable', 'status' => 'active']);
        $this->getJson("/api/products/{$other->id}/variants/{$variantId}")->assertNotFound();
        $this->getJson('/api/products/999/variants')->assertNotFound();
        $this->deleteJson("/api/products/{$product->id}/variants/{$variantId}")->assertNoContent();
        $this->getJson("/api/products/{$product->id}/variants/{$variantId}")->assertNotFound();
    }
    ...
    public function test_simple_and_variable_product_rules(): void
    {
        ...
        $this->actingAs($this->admin)->postJson("/api/products/{$simple->id}/variants", $this->variantPayload('SIMPLE-SKU', [$value->id]))
        ...
        $this->postJson("/api/products/{$variable->id}/variants", $this->variantPayload('VARIABLE-SKU', [$value->id]))->assertCreated();
        $this->patchJson("/api/products/{$variable->id}", [
    ...
    public function test_duplicate_sku_and_duplicate_attribute_combinations_return_409(): void
    {
        ...
        $this->actingAs($this->admin)->postJson("/api/products/{$product->id}/variants", $this->variantPayload('SKU-1', [$red->id, $small->id]))->assertCreated();
        $this->postJson("/api/products/{$product->id}/variants", $this->variantPayload('SKU-1', [$blue->id, $small->id]))
            ->assertConflict()->assertJsonPath('message', 'The SKU [SKU-1] is already in use.');
        $this->postJson("/api/products/{$product->id}/variants", $this->variantPayload('SKU-2', [$small->id, $red->id]))
    ...
    public function test_variant_validation_and_incorrect_attribute_relationships_are_rejected(): void
    {
        ...
        $this->actingAs($this->admin)->postJson("/api/products/{$product->id}/variants", [
        ...
        $this->postJson("/api/products/{$product->id}/variants", $this->variantPayload('BAD-COMBO', [$red->id, $blue->id]))
            ->assertConflict()->assertJsonPath('message', 'The variant attribute values must exist and contain one value per attribute.');
        $this->postJson("/api/products/{$product->id}/variants", $this->variantPayload('DUP-ID', [$red->id, $red->id]))
            ->assertUnprocessable()->assertJsonValidationErrors(['attribute_value_ids.1']);
        $this->postJson("/api/products/{$product->id}/variants", $this->variantPayload('MISSING-ID', [999]))
    ...
    public function test_update_variant_detects_sku_and_combination_conflicts(): void
    {
        ...
        $this->actingAs($this->admin)->patchJson("/api/products/{$product->id}/variants/{$second->id}", $this->variantPayload('FIRST', [$blue->id]))
            ->assertConflict();
        $this->patchJson("/api/products/{$product->id}/variants/{$second->id}", $this->variantPayload('SECOND', [$red->id]))
            ->assertConflict();
        $this->patchJson("/api/products/{$product->id}/variants/{$first->id}", $this->variantPayload('FIRST', [$red->id]))
            ->assertOk();
    }
