...
final class ReturnsApiTest extends TestCase
{
    ...
    public function test_customer_can_request_return_and_admin_can_approve_it(): void
    {
        ...
        $return = $this->actingAs($customer)->postJson('/api/customer/orders/'.$order->id.'/returns', ['reason' => 'Damaged', 'items' => [['order_item_id' => $item->id, 'quantity' => 1]]])->assertCreated()->assertJsonPath('data.status', 'pending')->json('data.id');
        $this->actingAs($admin)->getJson('/api/returns')->assertOk()->assertJsonCount(1, 'data');
        $this->actingAs($admin)->patchJson('/api/returns/'.$return.'/approve')->assertOk()->assertJsonPath('data.status', 'approved');
        $this->assertDatabaseHas('outbox_events', ['deduplication_key' => 'return:approved:'.$return, 'event_type' => 'order.return.approved']);
        $this->actingAs($customer)->postJson('/api/customer/orders/'.$order->id.'/returns', ['reason' => 'Again', 'items' => [['order_item_id' => $item->id, 'quantity' => 1]]])->assertStatus(409);
    }
    public function test_only_delivered_orders_and_valid_quantities_can_be_returned(): void
    {
        ...
        $this->actingAs($customer)->postJson('/api/customer/orders/'.$order->id.'/returns', ['reason' => 'Changed mind', 'items' => [['order_item_id' => $item->id, 'quantity' => 1]]])->assertStatus(409);
    }
