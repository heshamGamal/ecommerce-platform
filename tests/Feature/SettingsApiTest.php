...
class SettingsApiTest extends TestCase
{
    ...
    {
        $this->actingAs($this->admin)->putJson('/api/settings/store.name', [
        ...
        $this->putJson('/api/settings/store.enabled', [
            'group' => 'store', 'value' => true, 'type' => 'boolean',
        ])->assertOk()->assertJsonPath('data.value', true);
        $this->putJson('/api/settings/orders.limit', [
            'group' => 'orders', 'value' => 25, 'type' => 'integer',
        ])->assertOk()->assertJsonPath('data.value', 25);

        $this->getJson('/api/settings')->assertOk()->assertJsonCount(3, 'data');
        $this->getJson('/api/settings/groups/store')->assertOk()->assertJsonCount(2, 'data');
        $this->getJson('/api/settings/store.enabled')->assertOk()
    ...
    {
        $this->actingAs($this->admin)->getJson('/api/settings/missing')->assertNotFound()
            ->assertJsonPath('message', 'Setting [missing] was not found.');
        $this->getJson('/api/settings/groups/missing')->assertOk()->assertExactJson(['data' => []]);
    }

    public function test_setting_validation_covers_required_fields_and_type_specific_values(): void
    {
        $this->actingAs($this->admin)->putJson('/api/settings')->assertUnprocessable()
            ->assertJsonValidationErrors(['group', 'key', 'value', 'type']);
        $this->putJson('/api/settings/invalid.type', [
            'group' => 'test', 'value' => 'x', 'type' => 'unsupported',
        ])->assertUnprocessable()->assertJsonValidationErrors('type');
        $this->putJson('/api/settings/invalid.boolean', [
            'group' => 'test', 'value' => 'not-a-bool', 'type' => 'boolean',
        ])->assertUnprocessable()->assertJsonValidationErrors('value');
        $this->putJson('/api/settings/invalid.integer', [
            'group' => 'test', 'value' => 'one', 'type' => 'integer',
        ])->assertUnprocessable()->assertJsonValidationErrors('value');
        $this->putJson('/api/settings/invalid.json', [
    ...
    public function test_view_and_update_permissions_are_independent(): void
    {
        ...
        $this->actingAs($viewer)->getJson('/api/settings')->assertOk();
        $this->putJson('/api/settings/store.name', [
            'group' => 'store', 'value' => 'No', 'type' => 'string',
        ])->assertForbidden();
        $this->actingAs($updater)->getJson('/api/settings')->assertForbidden();
        $this->putJson('/api/settings/store.name', [
