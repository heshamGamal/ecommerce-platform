...
return Application::configure(basePath: dirname(__DIR__))
 ...
 ->withExceptions(function(Exceptions $exceptions):void{
  ...
  $exceptions->render(function(PaymentAlreadyProcessedException|InvalidOrderStatusTransitionException|OrderActionNotAllowedException $e,Request $r){if($r->is('api/*'))return response()->json(['message'=>$e->getMessage()],409);});
  $exceptions->render(function(PaymentFailedException|InvalidShippingAddressException|ShippingException $e,Request $r){if($r->is('api/*'))return response()->json(['message'=>$e->getMessage()],422);});
  $exceptions->render(function(PaymentException $e,Request $r){if($r->is('api/*'))return response()->json(['message'=>$e->getMessage()],422);});
  $exceptions->render(function(PaymentNotFoundException|ShippingRateNotFoundException|OrderNotFoundException|ProductNotFoundException|VariantNotFoundException|AttributeNotFoundException|AttributeValueNotFoundException|BrandNotFoundException|CategoryNotFoundException|SettingsNotFoundException|CustomerNotFoundException|CustomerFeatureNotFoundException|AddressNotFoundException|StaffNotFoundException|InventoryNotFoundException $e,Request $r){if($r->is('api/*'))return response()->json(['message'=>$e->getMessage()],404);});
